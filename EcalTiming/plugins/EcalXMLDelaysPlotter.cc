...
//========================================================================
	...
	for (int idcc = 1; idcc < 55; ++idcc) {
		if ( idcc == 10 ) {
			idcc = 46; /*iz=1;*/
		} //Just because I only care about EE
		for (int iTT = 1; iTT < 44; ++iTT) {
			...
			for (int istp = 1; istp < 6; ++istp) {
				for (int ich = 1; ich < 6; ++ich) {
					...
					if ( idcc > 45) {
						EEPXMLProfile_->Fill(ix, iy, XMLTTVals_[idcc - 1][iTT - 1]);
						EEPXMLProfileCh_->Fill(ix, iy, XMLTTVals_[idcc - 1][iTT - 1]);
					} else {
						EEMXMLProfile_->Fill(ix, iy, XMLTTVals_[idcc - 1][iTT - 1]);
						EEMXMLProfileCh_->Fill(ix, iy, XMLTTVals_[idcc - 1][iTT - 1]);
					}
				}
			}
		}
...
void EcalXMLDelaysPlotter::readinXMLs(void)
{
	...
	for ( int fed = 601; fed < 655 ; ++fed) {
		...
		if( !(FileXML.is_open()) ) {
			std::cout << "Error: file" << Form("sm_%d.xml", fed) << " not found!!" << std::endl;
			return;
		}
		char Buffer[5000];
		//int TimeOffset[71];
		...
		while( !(FileXML.eof()) ) {
			...
			if( initial.find("<DELAY_OFFSET>") != std::string::npos ) {
				...
				if(TT < 1 || TT > 68) {
					std::cout << "Ignoring TT " << TT << std::endl;
				} else {
