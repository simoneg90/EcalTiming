...
RecHitSelector::RecHitSelector(const edm::ParameterSet& iConfig):
	...
	_maxChi2(iConfig.getParameter<double>("maxChi2"))
{
   ...
   using namespace edm;
	...
	for(auto  recHit_itr : *RecHitHandle) {
		...
		if(recHit_itr.energy() < _minRecHitEnergy) continue;
                if(recHit_itr.timeError() < 0 || recHit_itr.timeError() > _maxTimeError) continue;
                if(recHit_itr.chi2() > _maxChi2) continue;
